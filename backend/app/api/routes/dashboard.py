...
from app.services.account_scope import is_platform_admin, scope_query
...
def summary(
    ...
    if is_platform_admin(current_user):
        post_counts = {status: count for status, count in db.query(Post.status, func.count(Post.id)).group_by(Post.status)}
        calendar_counts = {
            status: count for status, count in db.query(ContentCalendar.status, func.count(ContentCalendar.id)).group_by(ContentCalendar.status)
        }
        latest_logs = db.query(PublishLog).order_by(PublishLog.attempted_at.desc()).limit(10).all()
        upcoming_posts = db.query(Post).filter(Post.status == "scheduled").count()
        failed_publishes = db.query(PublishLog).filter(PublishLog.status == "failed").count()
